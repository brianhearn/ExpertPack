...
- [ ] **EP MCP `requires:` expansion** — implement the retrieval post-processor that expands a matched atom's context to include its `requires:` chain (depth 2, count 3, token-budget capped). Needed for v4.1 atomic semantics to actually land end-to-end.
