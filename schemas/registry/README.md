...
These `.spec.yaml` files are ExpertPack-native descriptive specs, not JSON Schema 2020-12 or OpenAPI documents. They are intended for humans, agents, exporters, and custom validators. If strict standards-based validation becomes a requirement, add separate `.schema.json` files and CI validation rather than overloading these descriptive specs.
